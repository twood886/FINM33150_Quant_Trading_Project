...
BIZD_Weekly_Vol <- BIZD_adj_close %>%
  dplyr::mutate(`BIZD` = `BIZD` / lag(`BIZD`, 7) - 1) %>%
  dplyr::filter(`date` >= as.Date("2017-12-31")) %>%
  dplyr::filter(`date` <= as.Date("2023-12-31")) %>%
  dplyr::filter(weekdays(`date`) == "Wednesday") %>%
  summarise(`sd` = sd(`BIZD`) * sqrt(52))
...
# Optimal weights are calculated on Wednesday using historic daily returns
Optimal_Weights <- 
  dplyr::left_join(
  ...
  dplyr::filter(weekdays(`date`) == "Wednesday") %>%
  #dplyr::mutate(`expanding_return_weekly` = purrr::accumulate(`return_weekly`, bind_rows)) %>%
  dplyr::filter(`date` >= as.Date("2017-12-31")) %>%
  # Calculate the optimal weights
  dplyr::mutate(
    `ols_expand_weights` = purrr::map(`expanding_return`, ols_weights, "BIZD"),
    `ols_expand_exp_weights` = purrr::map(`expanding_return`, ols_weights, "BIZD", lamda = 0.9),
    `ols_box_weights` = purrr::map(`box_return_252`, ols_weights, "BIZD"),
    `mgarch_weights` = purrr::map(`expanding_return`, mgarch_weights, "BIZD", fwd = 7, .progress = T))
...
Optimal_Weights_Returns <-
  ...
  dplyr::mutate(
    across(
      contains("_weights"),
      ~map2(`return_weekly`, lag(.x, 1), hedgefit), 
      .names = "{.col}_hedge_return")) %>%
  dplyr::rename_with(~str_remove(.x, "_weights"), .cols=contains("hedge_return")) %>%
  # Calculate the returns from the target portfolio with perfect costless collar
  dplyr::mutate(
    across(
      contains("_weights"),
      ~map2(`return_weekly`, lag(.x, 1), hedgefit_collar, collar_moneyness = c(.99, 1.01)), 
      .names = "{.col}_collar_return")) %>%
  dplyr::rename_with(~str_remove(.x, "_weights"), .cols=contains("collar_return"))

Optimal_Weights_Summary <-
  Optimal_Weights_Returns %>%
  select(c(`date`, contains("hedge_return"), contains("collar_return"))) %>%
  unnest(everything()) %>%
  pivot_longer(cols = c(everything(), -`date`),names_to = "return_metric", values_to = "return") %>%
  summarise(
    "Mean" = mean(`return`, na.rm =T) * 52,
    "MSE" = mean(`return`^2, na.rm = T),
    "SD" = sd(`return`, na.rm = T) * sqrt(52),
    "Downside_Vol" = sd(ifelse(`return`>0, 0, `return`), na.rm =T) * sqrt(52),
