...
Option_Assets <- options_data %>%
  ...
  dplyr::mutate(`option_asset` = map(`option_data`, WRDSOption)) %>%
...
BIZD_trade_target_close <- newTradeTarget(
  QuandlEquity("BIZD", BIZD),
  trade_date = as.Date("2023-02-04"),
  side = "Close",
  pct_capital = 1)

trade_targets <- Option_trade_targets + BIZD_trade_target_open + BIZD_trade_target_close
```

```{r}
dates <- BIZD %>%
  dplyr::filter(between(`date`, as.Date("2018-01-05"), as.Date("2023-02-04"))) %>%
  pull(`date`)
