...
test <- 
  ...
  dplyr::left_join(
    ...
    by = "date") %>%
  mutate(
    `exp_returns` = purrr::accumulate(`return_daily`, bind_rows),
    `roll_returns_252` = purrr::accumulate(`return_daily`, roll_bind, 252)) %>%
  filter(`date` >= as.Date("2017-12-31")) %>%
  mutate(
    `ols_expand_resid` = map2(lag(`exp_returns`, 7), `return_weekly`, ols_hedgefit, y = "BIZD"),
    `ols_expand_resid_exp` = map2(lag(`exp_returns`, 7), `return_weekly`, ols_hedgefit, y = "BIZD", lamda = .9),
    `ols_box_resid` = map2(lag(`roll_returns_252`, 7), `return_weekly`, ols_hedgefit, y = "BIZD"),
    `ols_box_resid_exp` = map2(lag(`roll_returns_252`, 7), `return_weekly`, ols_hedgefit, y = "BIZD", lamda = .9))
...
fit_test <-
  ...
  summarise(
    ...
    "SD" = sd(`residual`, na.rm = T) * sqrt(52),
    .by = `hedge_type`)
