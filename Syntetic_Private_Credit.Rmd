...
test <- 
  ...
  dplyr::mutate(
    `ols_expand_weights` = purrr::map(`expanding_return`, ols_coeff, "BIZD"),
    `ols_expand_exp_weights` = purrr::map(`expanding_return`, ols_coeff, "BIZD", lamda = 0.9),
    `ols_box_weights` = purrr::map(`box_return_252`, ols_coeff, "BIZD")) %>%
  # Use only Wednesdays
  dplyr::filter(weekdays(`date`) == "Wednesday") %>%
  # Calculate the returns from the target portfolio
  dplyr::mutate(
    across(
      c(`ols_expand_weights`, `ols_expand_exp_weights`, `ols_box_weights`),
