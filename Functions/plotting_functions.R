...
makeReturnPlots <- function(name, daily_data, datecol, pricecol=NULL, returncol = NULL){
  ...
  index_plots <- map2(returns_data, names(returns_data), ~plot_index(name, .y, .x))
...
plot_index <- function(name, frq, data){
  ...
  subtitle <- paste(
    "Data from",
    as.character(start.date, format = "%b %d, %Y"),
    "to",
    as.character(end.date, format = "%b %d, %Y"),
    paste0("(", frq, ")"))
...
plot_return_acfpcf <- function(name, frq, data){
  ...
  p <- d %>%
    ...
    theme_classic() +
    geom_hline(yintercept=ci.line.pcf, color="red", linetype="dashed") +
    geom_hline(yintercept=-ci.line.pcf, color="red", linetype="dashed") +
    labs(
